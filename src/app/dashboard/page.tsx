...
export default function DashboardPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto p-6 bg-white min-h-screen">
      ...
      {/* 投稿一覧 */}
      <h2 className="text-xl font-bold text-gray-900 mb-4">投稿一覧</h2>
      {posts.length === 0 ? (
                  ...
                  <div className="space-y-2 text-sm text-gray-800">
                    ...
                    {post.memo?.answerWhy && (
                      <div>
                        ...
                        <span className="whitespace-pre-wrap">{post.memo.answerWhy}</span>
                      </div>
                    )}
                    {post.memo?.answerWhat && (
                      <div>
                        <span className="font-bold">何が: </span>
                        <span className="whitespace-pre-wrap">{post.memo.answerWhat}</span>
                      </div>
                    )}
                    {post.memo?.answerNext && (
                      <div>
                        <span className="font-bold">次に: </span>
                        <span className="whitespace-pre-wrap">{post.memo.answerNext}</span>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-gray-800">（内容なし）</p>
                )}
              </li>
            );
          })}
